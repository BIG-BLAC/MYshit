...
* `Adafruit INA219 High Side DC Current Sensor Breakout <https://www.adafruit.co
m/product/904>`_
...
* Adafruit's Bus Device library: https://github.com/adafruit/Adafruit_CircuitPyt
hon_BusDevice
"""
...
class INA219:
    ...
    # bus_voltage                 RO : bus voltage (V- to GND) scaled to volts (
==load voltage)
    # current                     RO : current through shunt, scaled to mA
    # power                       RO : power consumption of the load, scaled to
Watt
    # set_calibration_32V_2A()    Initialize chip for 32V max and up to 2A (defa
ult)
    ...
    #   reset                     WO : Write Reset.RESET to reset the chip (must
 recalibrate)
    #   bus_voltage_range         RW : Bus Voltage Range field (use BusVoltageRa
nge.XXX constants)
    #   gain                      RW : Programmable Gain field (use Gain.XXX con
stants)
    #   bus_adc_resolution        RW : Bus ADC resolution and averaging modes (A
DCResolution.XXX)
    #   shunt_adc_resolution      RW : Shunt ADC resolution and averaging modes
(ADCResolution.XXX)
    #   mode                      RW : operating modes in config register (use M
ode.XXX constants)

    # raw_shunt_voltage           RO : Shunt Voltage register (not scaled)
    # raw_bus_voltage             RO : Bus Voltage field in Bus Voltage register
 (not scaled)
    # conversion_ready            RO : Conversion Ready bit in Bus Voltage regis
ter
    ...
    # calibration                 RW : calibration register (note: value is cach
ed)
    ...
    def calibration(self, cal_value: int) -> None:
        self._cal_value = cal_value  # value is cached for ``current`` and ``pow
er`` properties
    ...
    def set_calibration_32V_2A(self) -> None:
        """Configures to INA219 to be able to measure up to 32V and 2A of curren
t. Counter
        ...
        # VSHUNT_MAX = 0.32          (Assumes Gain 8, 320mV, can also be 0.16, 0
.08, 0.04)
        ...
        # 7. Compute the maximum current and shunt voltage values before overflo
w
        #
    ...
    def set_calibration_32V_1A(self) -> None:
        """Configures to INA219 to be able to measure up to 32V and 1A of curren
t. Counter overflow
        occurs at 1.3A.
...
""
        ...
        # VBUS_MAX = 32V        (Assumes 32V, can also be set to 16V)
        # VSHUNT_MAX = 0.32    (Assumes Gain 8, 320mV, can also be 0.16, 0.08, 0
.04)
        # RSHUNT = 0.1            (Resistor value in ohms)
...
w
        ...
        # End If
        #
        # ... In this case, we're good though since Max_Current is less than Max
Possible_I
        #
        # Max_ShuntVoltage = Max_Current_Before_Overflow * RSHUNT
    ...
    def set_calibration_16V_400mA(self) -> None:
        """Configures to INA219 to be able to measure up to 16V and 400mA of cur
rent. Counter
...
w
    ...
    def set_calibration_16V_5A(self) -> None:
        """Configures to INA219 to be able to measure up to 16V and 5000mA of cu
rrent. Counter
